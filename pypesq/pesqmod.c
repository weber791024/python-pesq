...
the terms 锟絇erceptual Evaluation of Speech Quality Algorithm锟�
and 锟絇ESQ Algorithm锟� refer to the objective speech quality
                   ...
                   ERROR_INFO *err_info)
{
    ...
    for (Utt_num = 0; Utt_num < err_info->Nutterances; Utt_num++)
        if ((err_info->Utt_End[Utt_num] - err_info->Utt_Start[Utt_num]) > Largest_uttsize)
            Largest_uttsize =
                     ...
                     ERROR_INFO *err_info, float *ftmp)
{
    ...
    for (Utt_id = 0; Utt_id < err_info->Nutterances; Utt_id++)
        if ((err_info->Utt_End[Utt_id] - err_info->Utt_Start[Utt_id]) > Largest_uttsize)
            Largest_uttsize =
...
void intensity_warping_of(float *loudness_dens, int frame, float *pitch_pow_dens)
{
    ...
    for (band = 0; band < Nb; band++)
    {
        ...
        if (input > threshold)
        {
            loudness_dens[band] = (float)(pow(threshold / 0.5, modified_zwicker_power) * (pow(0.5 + 0.5 * input / threshold, modified_zwicker_power) - 1));
        }
        else
        {
                                    ...
                                    const float *const pitch_pow_dens_deg)
{
    ...
    for (i = 0; i < Nb; i++)
    {
        ratio = (pitch_pow_dens_deg[frame * Nb + i] + (float)50) / (pitch_pow_dens_ref[frame * Nb + i] + (float)50);
...
#define MAX_NUMBER_OF_BAD_INTERVALS 99999
    ...
    } while ((sum_of_5_samples < CRITERIUM_FOR_SILENCE_OF_5_SAMPLES) && (samples_to_skip_at_start < maxNsamples / 2));
    ...
    } while ((sum_of_5_samples < CRITERIUM_FOR_SILENCE_OF_5_SAMPLES) && (samples_to_skip_at_end < maxNsamples / 2));
...
#define SEARCH_RANGE_IN_TRANSFORM_LENGTH 4
    ...
    for (frame = 0; frame <= stop_frame; frame++)
    {
        ...
        if (stop_frame + 1 > 1000)
        {
            ...
            if (timeWeightFactor > (float)0.5)
                timeWeightFactor = (float)0.5;
